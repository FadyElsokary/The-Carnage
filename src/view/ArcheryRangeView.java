...
public class ArcheryRangeView extends JFrame implements ActionListener{
	...
	public ArcheryRangeView(City x,Game s)
	{
		v=x;
		f=s;
		p=s.getPlayer();
		...
		{
			if(v.getMilitaryBuildings().get(i) instanceof ArcheryRange)
			{
				b=(ArcheryRange)v.getMilitaryBuildings().get(i);
			}
		}
		...
		image=new JLabel();
		image.setIcon(new ImageIcon("ArcheryRange.png"));
		...
		nameInfo="Building Type: ArcheryRange";
	...
	public void actionPerformed(ActionEvent e) {
		...
		{
			try {
				p.build("ArcheryRange", v.getName());
		...
		else
		{
			...
			else
			{
				...
				{
					try {
						p.recruitUnit("Archer", v.getName());
					} catch (BuildingInCoolDownException e1) {
						JOptionPane.showMessageDialog(null, "Building is Cooling Down");
