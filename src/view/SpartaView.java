...
public class SpartaView extends JFrame implements ActionListener{
	...
	public SpartaView(City x,Game g)
	{
		...
		image=new JLabel();
		image.setIcon(new ImageIcon("Sparta Final.png"));
		image.setBounds(0, 0, 1820, 900);
		
		
		
		archeryRange=new JButton("Archery Range");
		archeryRange.setBounds(700, 70, 250, 40);
		...
		barracks.setBounds(800, 350, 250, 40);
		...
		stable.setBounds(100, 500, 250, 40);
		...
		market.setBounds(100, 200, 250, 40);
									...
									if(e.getSource()==endTurn)
								  {
									...
									else
									{
										if(h!=null)
										{
											for(int i=0; i<f.getAvailableCities().size(); i++)
											{
												if(h.getTarget().equals(f.getAvailableCities().get(i).getName()))
												{
													f.endTurn();
													if(f.isGameOver())
													{
